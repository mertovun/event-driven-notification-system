...
- The dev seed key is **not a security boundary**. It is documented as such in `README.md`; production deploys must rotate it before exposing the API. A startup check logs a warning if the seed key's prefix is still present in `api_keys` and `APP_ENV != "development"`.
